  ...
  export default {
    ...
    methods: {
      ...
      handleTagClick () {
        ...
        this.dbTags.forEach(e => {
        if(e.propName===this.addTagInput.propName){
            this.dbTagValues = e.propValue
        }
      })
      },
      // 关闭标签 --删除
      ...
      handleInputConfirm (tagIndex) {
        ...
        let tagItem = {propId: index === -1 ? 0 : this.skuTags[tagIndex].tagItems[index].id, propValue: itemValue, valueId: index === -1 ? 0 : (maxValue + 1)}
        if (tagItem) {
